...
namespace NWH.VehiclePhysics2.Input
{
    ...
    public partial class VehicleInputHandler : VehicleComponent
    {
        ...
        /// <summary>
        /// Used to modify the input values.
        ...
        public float Steering
        {
            ...
            set
            {
                ...
                if (value < -0.3f)
                    steer = 1;
                else if (-0.3f < value && value < -0.2f)
                    steer = 2;
                else if (-0.2f < value && value < -0.1f)
                    steer = 3;
                else if (-0.1f < value  && value < 0.1f)
                    steer = 4;
                else if (0.1f < value && value < 0.2f)
                    steer = 5;
                else if (0.2f < value && value < 0.3f)
                    steer = 6;
                else if (0.3f < value)
                    steer = 7;
            }
        ...
        {
            get { return swapInputInReverse && vehicleController.powertrain.transmission.Gear < 0; }
        }
        ...
        public override bool VC_Disable(bool calledByParent)
        {
            ...
            {
                states.Reset();
                _inputSwappedBrakes = 0f;
                _inputSwappedThrottle = 0f;
                return true;
            }
    ...
    public partial class VehicleInputHandlerDrawer : ComponentNUIPropertyDrawer
    {
        ...
        public override bool OnNUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (Application.isPlaying && vc != null && vc.input != null)
            {
                ...
                drawer.Label($"Input Swapped Throttle: {vc.input.InputSwappedThrottle}");
                drawer.Label($"Input Swapped Brakes: {vc.input.InputSwappedBrakes}");
