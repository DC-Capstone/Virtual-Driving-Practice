...
namespace NWH.VehiclePhysics2
{
    ...
    public partial class VehicleController : Vehicle
    {
        ...
        public virtual void Update()
        {
            ...
            {
                if (btn.GetComponent<UnityEngine.cshbuttondown>().first == true)
                {
                    Display = false;
                }
            ...
            if (UnityEngine.Input.GetKey(KeyCode.KeypadEnter))
            {
        ...
        private void Start()
        {
            ...
            // Initialize components and load the states if not already initialized.
            if (!_isInitialized)
            {
                _componentCount = Components.Count;
                for (int i = 0; i < _componentCount; i++)
                {
                    VehicleComponent c = _components[i];
                    c.VC_SetVehicleController(this);
                }
            
                foreach (VehicleComponent c in _components)
                {
                    c.VC_LoadStateFromStateSettings();
                }
            }
